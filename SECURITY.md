...
Community foundation denies direct membership/role/audit writes, verifies live database roles inside transactional procedures and serializes competing transitions. Only an owner can transfer ownership; owners must transfer before leaving. Lower-ranked staff cannot override higher-ranked bans. Private communities are unlisted, with member-only metadata and staff-only audit records. Real database tests cover self-promotion, private requests, hierarchy, ban/approval races and concurrent ownership transfers. Community content/moderation UI remains an unreleased follow-up.
