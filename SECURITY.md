...
Social persistence enforces author identity, visibility and ownership in PostgreSQL. Trigger helpers are not directly callable, internal counters are inaccessible, and a private aggregate helper exposes only authenticated sports follow totals. Rate budgets serialize concurrent writes and survive content deletion. Direct PostgREST adversarial tests cover these boundaries; UI/server validation will be added with the social application milestone. Public interaction counts are visibility-filtered, not an unrestricted total.
